...
import type { LanguageMode, RenderedFile } from "./types.js";
...
function parseArgs(argv: string[]): { options: CliOptions } | { error: string } {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      ...
      if (!value) return { error: "--root requires a path" };
      options.root = resolve(value);
      index += 1;
    } else if (arg === "--lang") {
      const value = argv[index + 1];
      if (!isLanguageMode(value)) return { error: `Unsupported language: ${value ?? ""}` };
...
function renderStdout(files: RenderedFile[]): string {
